...
_user_config="$(krico_env_get USER_CONFIG)"
if [[ -z "${_user_config}" ]]; then
  ...
  if [[ ! -d "${_user_config}" ]]; then
    echo "ERROR: user_config.bash: USER_CONFIG directory '${_user_config}' doesn't exist" >&2
    return 1
  fi
fi
